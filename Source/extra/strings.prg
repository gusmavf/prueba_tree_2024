...
STATIC aStrings   := { ;
                       ...
                       { "New from Actual File", "Nuevo desde Fichero Actual", "New from Actual File", "New from Actual File", "New from Actual File", "New from Actual File" }, ;
                       ...
                       { "PDF files | *.pdf |", "ficheros PDF | *.pdf |", "fichier PDF | *.pdf |", "arquivos PDF | *.pdf |", "PDF-Dateien | *.pdf |", "archivi PDF | *.pdf |" }, ;
                       ...
                       { "Close", "Cerrar", "Fermer", "Fechar", "Alle", "Chiudi" }, ;
                       { "Close All", "Cerrar todas", "Fermer toutes", "Fecha todas", "Alle schließen", "Chiudi tutto" }, ;
                       { "Close all open windows", "Cerrar todas las ventanas abiertas", "Fermer toutes les fenêtres ouvertes", "Fechar todas as janelas abertas", "Alle offenen Fenster schließen", "Chiudi tutte le finestre aperte" }, ;
                       ...
                       { "must be returned from the expression", "debe ser devuelto de la expresión", "doit être retourné de l'expression", ;
                         "deve ser devolvido pela expressão", "Ausdruck-Rückgabewert", "deve essere ritornato dall'espressione" }, ;
                       { "done", "hecho", "terminé", "feito", "erledigt", "fatto" }, ;
                       { "The clipboard is not available now!", "El portapapeles no está disponible ahora!", ;
                         "Le presse-papiers n'est pas disponible maintenant!", "A área de transferência não está disponível!", ;
                         "Zur Zeit kein Zugriff auf die Zwischenablage möglich", "Clipboard non disponibile!" }, ;
                       { "Properties", "Propiedades", "Propriétés", "Propriedades", "Eigenschaften", "Proprietà" }, ;
                       { "Help file", "Fichero de Ayuda", "Fichier d´aide", "Arquivo de ajuda", "Hilfedatei", "File di Help" }, ;
                       { "User", "Usuario", "Usager", "User", "Benutzer", "Utente" }, ;
                       { "Only Compile", "Sólo Compilar", "Seulement Compiler", "Única Compilando", "Nur Kompilieren", "Solo Compilazione" }, ;
                       { "Compile", "Compilar", "Compiler", "Compilando", "Kompilieren", "Compilazione" }, ;
                       { "Insert Code", "Insert Code", "Insérer Code", "Insira Código", "Code Einfügen", "Inserisce Codice" }, ;
                       { "Create a new file", "Crear un nuevo fichero", "Creer un nouveau fichier", "Criar um novo arquivo", "Neue Datei in einem", "Creare un nuovo file" }, ;
                       { "Import Project", "Importar Proyecto", "Importer Projet", "Importar Projeto", "Import Projekt", "Importa Progetto" }, ;
                       { "Projects Recents", "Proyectos Recientes", "Projets Récents", "Projetos Recentes", "Jüngsten Projekten", "Progetti Recenti" }, ;
                       { "Historic", "Historico", "Historique", "Historico", "Historisch", "Storico" }, ;
                       { "Transform", "Transformar", "Transformer", "Transformar", "Umwandeln", "Trasformare" }, ;
                       { "Transform to", "Transformar a", "Transformer", "Transformar", "Umwandeln", "Trasformare" }, ;
                       { "Centered", "Centrado", "Centrer", "center", "Mitte", "Centro" }, ;
                       { "Setting Tools", "Configurar Herramientas", "Setting Tools", "Setting Tools", "Setting Tools", "Setting Tools" }, ;
                       { "Setting Links", "Configurar Enlaces", "Setting Links", "Setting Links", "Setting Links", "Setting Links" }, ;
                       { "Setup", "Configuración", "Configuration", "Configurar", "Drucker", "Configurare" }, ;
                       { "Margins", "Margenes", "Margins", "Margins", "Margins", "Margins" }, ;
                       { "As Viewed", "Como se ve", "Tel que vu", "Wie gesehen", "As Viewed", "As Viewed" }, ;
                       { "Crop", "Recortar", "Couper", "Cut", "Crop", "Crop" }, ;
                       { "Fit Height", "Ajustar altura", "Ajuster la hauteur", "Höhe anpassen", "Fit Height", "Fit Height" }, ;
                       { "Fit In Rect", "Ajustar al rectángulo", "Ajuster au rectangle", "Für Rechteck anpassen", "Fit In Rect", "Fit In Rect" }, ;
                       { "Fit Width", "Ajustar anchura", "Ajuster la largeur", "Breite anpassen", "Fit Width", "Fit Width" }, ;
                       { "Original Image", "Imagen original", "Image originale", "Originalbild", "Original Image", "Original Image" }, ;
...
FUNCTION FWString( cString )
   ...
   */
   ...
   IF ( ValType( cString ) == 'N' )
      //388
      cRet := aStrings[ 388 + cString ][ nLanguage ]
      RETURN cRet
   ENDIF

   cSeek    := Upper( AllTrim( cString ) )
   IF ( nAt := AScan( aStrings, { | aString | Upper( aString[ 1 ] ) == cSeek } ) ) != 0
      IF Len( aStrings[ nAt ] ) >= nLanguage
         cRet  := aStrings[ nAt ][ nLanguage ]
      ENDIF
      IF cRet == NIL
         AddMissingString( cString )
         cRet  := cString
      ENDIF
      RETURN cRet
   ENDIF
   ...
   cSeek    := CharRem( "&", cSeek )
   IF ( nAt := AScan( aStrings, { | aString | Upper( CharRem( "&", aString[ 1 ] ) ) == cSeek } ) ) != 0
      IF Len( aStrings[ nAt ] ) >= nLanguage
         cRet  := aStrings[ nAt ][ nLanguage ]
      ENDIF
      IF cRet == NIL
   ...
   IF nLanguage <= 6
      IF lSilent
//         cMsg  := '"' + cString + '" not found in Strings table'
//         FWLOG cMsg
      ELSE
         MsgInfo( FWString( "The string" ) + ': "' + cString + '" ' + ;
                  FWString( "for language" ) + " " + ;
                  { "EN", "ES", "FR", "PT", "DE", "IT", "7", "8", "9", "10" }[ nLanguage ] + CRLF + ;
...
FUNCTION FWSetLanguage( nNewLanguage )
   ...
   IF ! Empty( nNewLanguage )
      ...
      // FW_N2WSetUp() // set up lang strings for num2words() function
   ENDIF
...
FUNCTION FWAddLanguage( aLang, nLang )
   ...
   CheckArray()
   IF ValType( aLang ) == 'A' //.and. ! Empty( aLang ) // commented out FWH1808
...
STATIC FUNCTION CheckArray()
   ...
   LOCAL nArrayLen   := Len( aStrings )
   LOCAL nSize       := Len( aStrings[ 1 ] )
   LOCAL n

   FOR n := 1 TO nArrayLen
      IF Len( aStrings[ n ] ) > nSize
         ASize( aStrings[ n ], nSize )
      ENDIF
   NEXT
...
STATIC FUNCTION AddMissingString( cString )

   IF AScan( aMissing, { | c | c == cString } ) == 0
...
FUNCTION FWMissingStrings()
   ...
   IF lSilent
      ...
      ELSE
         ...
         aMissed     := AClone( aMissing )
         ASort( aMissed, NIL, NIL, { | x, y | Upper( CharRem( "&", x ) ) < Upper( CharRem( "&", y ) ) } )
         AEval( aMissed, { | x, i | aMissed[ i ] := '{ "' + x + '", }' } )
         cResult  := "   FWSetString( nLang, { ;" + CRLF + "      " + ;
                                               FW_ArrayAsList( aMissed, ", ;" + CRLF + "      " ) + ;
                                               " ;" + CRLF + "   } )" + CRLF

      ENDIF
      MemoWrit( "missing.str", cResult )

   ELSEIF ! Empty( aMissing )

      AEval( aMissing, { | cString | cResult += Space( 7 ) + '{ "' + cString + ;
                                                                '", "", "", "", "", "" },;' + CRLF } )
      FW_MEMOEDIT( cResult, FWString( "Copy and paste in" ) + ;
                   " FWH\source\function\strings.prg" )
   ENDIF

   RETURN aMissing //cResult
...
//----------------------------------------------------------------------------//
// IMPROVED FWH1808 : 2018-11-06

FUNCTION FWAddString( aString )

   LOCAL c, nAt

   IF HB_ISARRAY( aString ) .and. ! Empty( aString )
      IF HB_ISARRAY( aString[ 1 ] ) .or. ;
         ( HB_ISCHAR(  aString[ 1 ] ) .and. "|" $ aString[ 1 ] .and. ! ( "*." $ aString[ 1 ] ) )

         AEval( aString, { | a | FWAddString( a ) } )
         RETURN aStrings
      ENDIF
      c     := Upper( AllTrim( aString[ 1 ] ) )
      IF( nAt := AScan( aStrings, { | a | Upper( a[ 1 ] ) == c } ) ) > 0
      AEval( aString, { | c, i | IF( c == NIL,, aStrings[ nAt, i ] := AllTrim( c ) ) }, 2, Len( aStrings[ nAt ] ) )
   ELSE
      IF Len( aString ) > Len( aStrings[ 1 ] )
         ASize( aString, Len( aStrings[ 1 ] ) )
      ENDIF
      AAdd( aStrings, aString )
   ENDIF
ELSEIF HB_ISCHAR( aString ) .and. "|" $ aString
   RETURN FWAddString( HB_ATokens( aString, "|" ) )
ENDIF
...
FUNCTION FWSetString( nLang, aString )

   LOCAL c, nAt, aNew

   IF Empty( nLang ) .or. nLang < 1 .or. nLang > Len( aStrings[ 1 ] )
      ? "Invalid language"
      RETURN NIL
   ENDIF
   ...
   IF HB_ISARRAY( aString ) .and. ! Empty( aString )
      IF HB_ISARRAY( aString[ 1 ] ) .or. ;
         ...
         AEval( aString, { | a | FWSetString( nLang, a ) } )
         RETURN aStrings
      ENDIF
      c     := Upper( AllTrim( aString[ 1 ] ) )
      IF( nAt := AScan( aStrings, { | a | Upper( a[ 1 ] ) == c } ) ) > 0
      aStrings[ nAt, nLang ]  := AllTrim( aString[ 2 ] )
   ELSE
      aNew           := Array( Len( aStrings[ 1 ] ) )
      aNew[ 1 ]      := aString[ 1 ]
      aNew[ nLang ]  := aString[ 2 ]
      AAdd( aStrings, aNew )
   ENDIF
ELSEIF HB_ISCHAR( aString ) .and. "|" $ aString
   RETURN FWAddString( HB_ATokens( aString, "|" ) )
